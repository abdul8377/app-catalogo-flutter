...
first_matches = list(first_test_pattern.finditer(filters_test))
if len(first_matches) != 1:
    fail(
        "No se pudo localizar de forma única la primera prueba de filtros. "
        f"Coincidencias: {len(first_matches)}."
    )

new_filter_test = """  testWidgets(
...
"""
filters_test = first_test_pattern.sub(new_filter_test, filters_test, count=1)
...
backup_dir = ROOT / (
    ".backup_lista_errores_v1_catalogo_carrito_clientes_v2_"
...
print(f"\nRespaldo: {backup_dir}")
print("\nAplicadas las correcciones v2 de Catálogo, Nuevo pedido, carrito y clientes.")
