...
TARGET = (
    ROOT
    / "lib/features/catalogo/presentation/widgets/"
    / "paso5_precios_corregido.dart"
)
...
if missing:
    fail(
        "El archivo no corresponde a la versión esperada de la fase 4B. "
...
for match in matches:
    ...
    looks_like_old_date_helper = (
        ...
        and "onPressed: onTap" in body
        and "onPressed: onClear" in body
        and "calendar_today_outlined" in body
        and "Quitar fecha final" in body
    )
...
if len(candidates) > 1:
    fail(
        "Se encontraron varios cuerpos huérfanos del antiguo selector "
        "de fecha. No es seguro eliminarlos automáticamente."
    )

if len(candidates) == 0:
    if matches:
        fail(
            "No se encontró el antiguo selector de fecha, pero existen otros "
            "miembros con la forma `) {`. Revisa el resultado de flutter analyze."
        )

    print(f"Verificado: {TARGET.relative_to(ROOT)}")
    print(
        "El cuerpo huérfano del selector de fecha ya no existe. "
        "La reparación de precios ya está aplicada."
    )
    print("\nNo se modificó ningún archivo.")
    print("Ejecuta ahora:")
...
if re.search(r"(?m)^\)\s*\{\s*$", updated):
    fail(
        "Después de preparar la reparación todavía queda un miembro "
        "huérfano con la misma forma."
    )
...
print("\nCuerpo huérfano del selector de fecha eliminado correctamente.")
print("Ejecuta:")
print("  dart format lib test")
print("  flutter test test/reglas_comerciales_producto_test.dart")
print("  flutter test test/flujo_producto_coherencia_test.dart")
