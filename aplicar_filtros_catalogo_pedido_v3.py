...
"""
catalog_filters_start = catalog_state.find(
    "class CatalogoFiltros extends Equatable {"
)
catalog_state_start = catalog_state.find(
    "class CatalogoState extends Equatable {"
)
if catalog_filters_start < 0 or catalog_state_start < 0:
    fail("No se pudieron delimitar CatalogoFiltros y CatalogoState.")
if catalog_filters_start >= catalog_state_start:
    fail("El orden de las clases en catalogo_state.dart no es el esperado.")

catalog_filters_block = catalog_state[
    catalog_filters_start:catalog_state_start
]
catalog_filters_block = replace_once(
    ...
    + "  @override\n"
    "  List<Object?> get props => [\n",
    "agregar copyWith a CatalogoFiltros",
)
catalog_state = (
    catalog_state[:catalog_filters_start]
    + catalog_filters_block
    + catalog_state[catalog_state_start:]
)
catalog_state = replace_once(
    catalog_state,
...
print(f"\nRespaldo: {backup_dir}")
print("\nFiltros jerárquicos de Catálogo y Nuevo pedido aplicados; CatalogoFiltros fue modificado de forma aislada y la prueba existente fue respaldada.")
print("Ejecuta:")
