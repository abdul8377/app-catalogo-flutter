...
""",
        """relation_panel_start_marker = "  Widget _relationCategoryPanel("
relation_panel_end_marker = "  void _appendRelationCategoryRows("
if design.count(relation_panel_start_marker) != 1:
    fail("No se encontró de forma única el panel de categorías por marca.")
if design.count(relation_panel_end_marker) != 1:
    fail("No se encontró el límite del panel de categorías por marca.")
relation_panel_start = design.index(relation_panel_start_marker)
relation_panel_end = design.index(
    relation_panel_end_marker,
    relation_panel_start,
)
design = (
    design[:relation_panel_start]
    + new_relation_category_panel.rstrip()
    + "\\n\\n"
    + design[relation_panel_end:]
)
""",
        "reemplazo acotado del panel de relaciones",
    ),
    (
        """design = replace_block(
    design,
...
""",
        """relation_rows_start_marker = "  void _appendRelationCategoryRows("
relation_rows_end_marker = "  Widget _relationSaveBar("
if design.count(relation_rows_start_marker) != 1:
    fail("No se encontraron de forma única las filas de relación.")
if design.count(relation_rows_end_marker) != 1:
    fail("No se encontró el límite de las filas de relación.")
relation_rows_start = design.index(relation_rows_start_marker)
relation_rows_end = design.index(
    relation_rows_end_marker,
    relation_rows_start,
)
design = (
    design[:relation_rows_start]
    + new_append_relation_rows.rstrip()
    + "\\n\\n"
    + design[relation_rows_end:]
)
""",
        "reemplazo acotado de filas de relación",
    ),
    (
        """design = replace_block(
    design,
...
""",
        """change_relation_start_marker = "  void _changeRelation("
change_relation_end_marker = "  void _saveRelations("
if design.count(change_relation_start_marker) != 1:
    fail("No se encontró de forma única el cambio de relación.")
if design.count(change_relation_end_marker) != 1:
    fail("No se encontró el límite del cambio de relación.")
change_relation_start = design.index(change_relation_start_marker)
change_relation_end = design.index(
    change_relation_end_marker,
    change_relation_start,
)
design = (
    design[:change_relation_start]
    + new_change_relation.rstrip()
    + "\\n\\n"
    + design[change_relation_end:]
)
""",
        "reemplazo acotado del cambio de relación",
    ),
)

for _old_operation, _new_operation, _operation_label in _relation_operations:
    phase_source = _phase_replace_once(
        phase_source,
        _old_operation,
        _new_operation,
...
print("\nFase 1 restaurada y reaplicada con formularios y relaciones acotados.")
print(
