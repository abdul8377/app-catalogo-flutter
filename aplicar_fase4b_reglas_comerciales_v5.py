...
# Botón solo en el editor de presentaciones, después del alcance "Disponible para".
scope_anchor = (
    "            _buildScopeEditor(\n"
    "              title: 'Disponible para',"
)
scope_count = sales.count(scope_anchor)
if scope_count != 1:
    fail(
        'No se pudo localizar de forma única el alcance de la presentación. '
        f'Se encontraron {scope_count} coincidencias.'
    )
...
scope_index = sales.index(scope_anchor)
button_index = sales.find(anchor, scope_index)
if button_index < 0:
    fail(
        'No se encontró el bloque de acciones del editor de presentación '
        'después de “Disponible para”.'
    )
...
# Precio exacto usa la regla resuelta para el variantId recibido.
...
if price_rule_anchor in prices_adapter:
...
elif "final rule = presentation.ruleFor(variantId);" not in prices_adapter:
    fail(
        'No se encontró el método _combinationFromPresentation compatible '
        'para resolver la regla comercial por variante.'
    )
...
print(f'\nRespaldo: {backup_dir}')
print('\nFase 4B aplicada con resolución comercial mediante variantId.')
print('Ejecuta:')
