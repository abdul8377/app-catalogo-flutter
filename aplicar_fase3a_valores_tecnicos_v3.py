...
database = regex_once(
    database,
    r"""(?m)^(?P<indent>[ \t]*)valor_texto TEXT,\r?
(?P=indent)valor_numero REAL,\r?
(?P=indent)valor_booleano INTEGER,\r?
(?P=indent)valor_normalizado REAL,\r?
""",
    r"""\g<indent>tipo_valor TEXT NOT NULL DEFAULT 'texto',
\g<indent>valor_texto TEXT,
\g<indent>valor_numero REAL,
\g<indent>valor_numero_hasta REAL,
\g<indent>valor_booleano INTEGER,
\g<indent>valor_normalizado REAL,
\g<indent>valor_normalizado_hasta REAL,
""",
...
database = regex_once(
    database,
    r"""(?m)^(?P<indent>[ \t]*)if \(oldVersion < 20\) \{\r?
(?P=indent)  await _migrarCodigoProveedorVariantes\(db\);\r?
(?P=indent)\}\r?
""",
    r"""\g<indent>if (oldVersion < 20) {
\g<indent>  await _migrarCodigoProveedorVariantes(db);
\g<indent>}
\g<indent>if (oldVersion < 21) {
\g<indent>  await _migrarValoresTecnicos(db);
\g<indent>}
""",
...
database = regex_once(
    ...
    r"""(?m)^(?P<indent>[ \t]*)\}\r?
\r?
(?P<method>[ \t]*Future<void> _migrarAtributosDef\(Database db\) async \{)""",
    r"""\g<indent>  await _asegurarUnidadesTecnicas(db);
\g<indent>}
...
print(f"\nRespaldo: {backup_dir}")
print("\nFase 3A aplicada con detección estructural v2.")
print("Ejecuta:")
