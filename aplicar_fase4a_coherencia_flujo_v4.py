...
if not TECHNICAL_PARSER.exists():
    fail(
        'No se encontró valor_tecnico_parser.dart. '
        'La fase 3A de valores técnicos no está aplicada.'
    )
...
page = regex_once(
    page,
    r"state\.editando\s*"
    r"\?\s*_EditarProductoScaffold\(state:\s*state\)\s*"
    r":\s*_RegistrarProductoScaffold\(state:\s*state\)",
...
if technical_import not in variants:
    variants = replace_once(
        variants,
        "import '../../domain/services/codigo_interno_generator.dart';\n",
        "import '../../domain/services/codigo_interno_generator.dart';\n"
        + technical_import,
        'importar parser técnico en lista',
    )

old_numeric_pattern = r"r'^[0-9\s/.,]*$'"
new_numeric_pattern = r"r'^[0-9\s/.,aA+\-–—]*$'"
old_filtering_pattern = (
    "FilteringTextInputFormatter.allow(RegExp(r'[0-9\\s/.,]'))"
)
new_filtering_pattern = (
    "FilteringTextInputFormatter.allow("
    "RegExp(r'[0-9\\s/.,aA+\\-–—]'))"
)

if old_numeric_pattern in variants:
    variants = replace_once(
        variants,
        old_numeric_pattern,
        new_numeric_pattern,
...
elif old_filtering_pattern in variants:
    variants = replace_once(
        variants,
        old_filtering_pattern,
        new_filtering_pattern,
...
elif new_numeric_pattern not in variants and new_filtering_pattern not in variants:
    fail(
        'No se encontró el filtro numérico de la lista de variantes '
        'para habilitar rangos.'
    )
variants = regex_once(
    variants,
...
print(f'\nRespaldo: {backup_dir}')
print('\nFase 4A aplicada con compatibilidad para la fase 3A y edición unificada.')
print('Ejecuta:')
