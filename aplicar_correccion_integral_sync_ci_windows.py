...
def resolve_tool(name: str) -> str:
    ...
    if executable is None:
        raise SystemExit(
            f"No se encontró '{name}' en PATH. Abre una terminal donde "
            "flutter --version y dart --version funcionen."
        )
    return executable
...
def run_windows_tool(
    ...
    *,
    ...
    print("> " + " ".join([executable, *arguments]))
...
def run_command(
    ...
    if result.returncode != 0:
        raise SystemExit(
            f"Falló: {executable} {' '.join(arguments)}"
        )
...
def main() -> None:
    ...
    print("\nCorrección aplicada y comandos Windows verificados.")
    print("\nRevisa y publica:")
